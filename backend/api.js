...
async function handleAPI(req, res) {
    ...
    if (url.pathname === '/api/disputes') {
        ...
        // Query: All CONTRADICE edges + their endpoints
        // Per spec: only show nodes with controversies, only show CONTRADICE edges
        const contradiceEdges = await ageQuery2(
            `MATCH (a)-[r:CONTRADICE]->(b) RETURN a, r, b`,
            ['a', 'r', 'b']
        ).catch(() => []);

        // Build node map and edge list
        for (const row of contradiceEdges) {
            const a = flat(row.a);
            ...
            if (a.id && !nodeMap.has(a.id)) {
                ...
                a.hasControversies = true;  // Per Allium spec
                a.controversyCount = 0;
                nodeMap.set(a.id, a);
            }
            ...
            if (b.id && !nodeMap.has(b.id)) {
                ...
                b.hasControversies = true;  // Per Allium spec
                b.controversyCount = 0;
                nodeMap.set(b.id, b);
            }

            // Increment controversy counts
            if (a.id) nodeMap.get(a.id).controversyCount++;
            if (b.id) nodeMap.get(b.id).controversyCount++;
        ...
        // No focalIds needed for controversy mode - it's a filter, not a focus
    ...
    // === GET NODE: obtener información completa de un nodo ===
    if (url.pathname === '/api/node' && req.method === 'GET') {
        const id = url.searchParams.get('id');
        if (!id) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end('{"error":"id parameter required"}');
            return;
        }
        ...
        try {
            ...
            if (nodes.length === 0) {
                ...
                res.end('{"error":"Node not found"}');
                return;
            }

            const node = flat(nodes[0]);
            res.writeHead(200, { 'Content-Type': 'application/json' });
...
Return ONLY a valid JSON array. Max 10 entities. If none are warranted, return [].`;
    ...
    // === VERIFICAR CONTRADICCIÓN (Fase 1: sin autenticación) ===
    if (url.pathname.startsWith('/api/disputes/') && url.pathname.endsWith('/verify') && req.method === 'POST') {
        const pathParts = url.pathname.split('/');
        const fromId = pathParts[3]; // /api/disputes/{fromId}/{toId}/verify
        const toId = pathParts[4];

        if (!fromId || !toId) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end('{"error":"Invalid path. Use /api/disputes/{fromId}/{toId}/verify"}');
            return;
        }

        const body = JSON.parse(await readBody(req));
        const { verifier, vote, confidence, comment } = body;

        // Validaciones
        if (!verifier || !vote) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end('{"error":"verifier and vote are required"}');
            return;
        }

        if (!['agree', 'disagree', 'uncertain'].includes(vote)) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end('{"error":"vote must be: agree, disagree, or uncertain"}');
            return;
        }
        ...
        if (conf < 0 || conf > 1) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end('{"error":"confidence must be between 0 and 1"}');
            return;
        }
        ...
        try {
            ...
            try {
                ...
                // 1. Verificar que existe la arista CONTRADICE
                const checkEdge = await client.query(`
                    SELECT * FROM cypher('lombardi', $$
                ...
                if (checkEdge.rows.length === 0) {
                    res.writeHead(404, { 'Content-Type': 'application/json' });
                    res.end('{"error":"Contradiction not found"}');
                    return;
                }

                const edge = parseAgtype(checkEdge.rows[0].r);
                const props = edge.properties;

                // 2. Actualizar contadores según el voto
                let voteAgreeCount = props.vote_agree_count || 0;
                let voteDisagreeCount = props.vote_disagree_count || 0;
                let voteUncertainCount = props.vote_uncertain_count || 0;

                if (vote === 'agree') voteAgreeCount++;
                else if (vote === 'disagree') voteDisagreeCount++;
                else voteUncertainCount++;

                // 3. Calcular consensus_score (sin ponderación por reputación en Fase 1)
                const totalDecisive = voteAgreeCount + voteDisagreeCount;
                let consensusScore = 0.5; // neutral por defecto
                if (totalDecisive > 0) {
                    consensusScore = voteAgreeCount / totalDecisive;
                }

                // 4. Determinar verification_status según thresholds
                let verificationStatus = 'pending';
                if (totalDecisive >= 3) { // mínimo 3 votos decisivos
                    if (consensusScore >= 0.75) {
                        verificationStatus = 'confirmed';
                    } else if (consensusScore <= 0.25) {
                ...
                // 5. Actualizar la arista
                await client.query(`
                    SELECT * FROM cypher('lombardi', $$
                        MATCH (a {id: '${esc(fromId)}'})-[r:CONTRADICE]->(b {id: '${esc(toId)}'})
                        SET r.vote_agree_count = ${voteAgreeCount},
                            r.vote_disagree_count = ${voteDisagreeCount},
                            r.vote_uncertain_count = ${voteUncertainCount},
                            r.consensus_score = ${consensusScore},
                            r.verification_status = '${verificationStatus}'
                        RETURN r
                    $$) as (r agtype)
                `);

                // 6. TODO Fase 2: Guardar en tabla verification_votes con user_id
                // Por ahora solo registramos en console para debugging
                console.log(`[VERIFY] ${verifier} voted "${vote}" (conf: ${conf}) on ${fromId} -> ${toId}`);
                if (comment) console.log(`  Comment: ${comment}`);

                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({
                    ok: true,
                    vote_agree_count: voteAgreeCount,
                    vote_disagree_count: voteDisagreeCount,
                    vote_uncertain_count: voteUncertainCount,
            ...
            console.error('Error in /api/disputes/verify:', err);
            res.writeHead(500, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: err.message }));
        }
        return;
